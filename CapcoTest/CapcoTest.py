...
if __name__ == "__main__":
    ...
    for _ in files:
        T = timeit.timeit(
            'asyncio.run(write_to_file({"A": 2}, f))', number=n_tests, globals=globals(),
        )
        t_ += T
    ...
    t_ = 0
    T = timeit.timeit(
        "asyncio.run(slow_method_wrapper(files))", number=n_tests, globals=globals(),
    )
    t_ += T
    print(time.perf_counter() - ts, t_)
    track.append(t_)
    ...
    t_ = 0
    T = timeit.timeit(
        "asyncio.run(slow_method_wrapper2(files))", number=n_tests, globals=globals(),
    )
    t_ += T
    print(time.perf_counter() - ts, t_)
    track.append(t_)
